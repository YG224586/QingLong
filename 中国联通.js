...
class CustomUserService extends UserServiceClass {
  ...
  async marketWateringTask() {
    try {
      ...
      const nextTriggerDate = wateringStatus.nextTriggerDate || "";
      
      // 判断今日是否还可以浇花
      const today = new Date();
      const nextTrigger = new Date(nextTriggerDate);
      const isNextDay = nextTrigger.toDateString() !== today.toDateString();
      
      // 如果已浇花次数小于总次数，且下次浇花时间不是明天，则可以浇花
      if (triggeredTime < triggerTime && !isNextDay) {
        this.log(`今日还可浇花，已浇花 ${triggeredTime}/${triggerTime} 次，开始执行浇花`, { notify: true });
        const success = await this.marketWatering();
        
        if (success) {
          // 浇水成功后，更新进度显示
          const newTriggeredTime = triggeredTime + 1;
          this.log(`权益超市浇花完成，当前进度: ${newTriggeredTime}/${triggerTime}`, { notify: true });
        } else {
          this.log("权益超市浇花失败");
        }
      } else {
        this.log(`今日浇花已完成，已浇花 ${triggeredTime}/${triggerTime} 次，下次浇花时间: ${nextTriggerDate}`, { notify: true });
      }
