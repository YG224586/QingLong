...
包含以下功能:
1. 每日自动签到 (签到/戳一戳/新版云朵领取)
2. 多账号并发支持 (环境变量 & 分隔)
3. 云朵中心新版任务自动处理 (上传/分享/AI相机/月任务补传等)
4. 算力大作战任务、领奖、抽奖与好友助力
5. 临时文件智能清理与详细日志推送
...
TOKENPK_MARKET_NAME = 'National_TokenPK'
TOKENPK_SOURCE_ID = '1030'
TOKENPK_PRIZE_PAGE_URL = 'https://m.mcloud.139.com/portal/cloudItem/index.html?path=getPrize&sourceid=1102'
TOKENPK_INVITE_CODE = 'eb11f7338ef147c48b39d92ec14e75bd2008'
...
def save_tokenpk_assist_state(account, invite_code, month):
    storage_path = get_device_id_storage_path()
    data = load_device_id_storage()
    account_data = data.get(account, {})
    account_data['tokenpkAssistCode'] = invite_code
    account_data['tokenpkAssistMonth'] = month
    data[account] = account_data
    try:
        with open(storage_path, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f'保存算力大作战助力状态失败: {e}')
...
class YP:
    ...
    def build_tokenpk_page_url(self):
        return (f'{self.market_base_url}/portal/yunClound/index.html?path={TOKENPK_MARKET_NAME}'
                f'&sourceid={TOKENPK_SOURCE_ID}&enableShare=1')

    def request_tokenpk_json(self, path_url, params=None, data=None, method='GET', headers=None, retries=3):
        url = path_url if path_url.startswith('http') else f'{self.market_base_url}{path_url}'
        request_headers = {'Cache-Control': 'no-cache'}
        if headers:
            request_headers.update(headers)
        return self.request_json(
            url,
            headers=self.build_market_headers(request_headers, referer=self.build_tokenpk_page_url()),
    ...
    @staticmethod
    def build_tokenpk_task_payload(task):
        app_button = (task.get('button') or {}).get('app') or {}
        return {
            'marketName': TOKENPK_MARKET_NAME,
            'taskId': task.get('id'),
            'key': app_button.get('ext', ''),
            'source': 'app',
        }

    def get_tokenpk_task_list(self, log_error=True):
        data = self.request_tokenpk_json('/ycloud/tokenpk/task/list', params={
            'marketName': TOKENPK_MARKET_NAME,
            'platform': 'ios',
            'sortState': 'true',
        })
        if not data:
            if log_error:
                self.log('-获取算力大作战任务失败: 接口无响应')
            return None
        if data.get('code') != 0:
            if log_error:
                self.log(f"-获取算力大作战任务失败: {data.get('msg', '未知错误')}")
            return None
        return data.get('result') or []

    def get_tokenpk_task(self, task_id):
        tasks = self.get_tokenpk_task_list(log_error=False)
        if tasks is None:
            return None
        return next((task for task in tasks if task.get('id') == task_id), None)

    @staticmethod
    def get_tokenpk_task_name(task):
        return re.sub(r'<[^>]+>', '', task.get('name') or task.get('description') or str(task.get('id', '')))

    def log_tokenpk_prizes(self, prizes, default_message):
        successful = [prize.get('prizeName', '') for prize in prizes or [] if prize.get('success')]
        if successful:
            self.log(f"-{default_message}: {' + '.join(filter(None, successful))}")
            return True
        errors = [prize.get('errorMsg', '') for prize in prizes or [] if prize.get('errorMsg')]
        self.log(f"-{default_message}: {errors[0] if errors else '成功'}")
        return not errors

    def receive_tokenpk_task_prize(self, task):
        task_name = self.get_tokenpk_task_name(task)
        data = self.request_tokenpk_json(
            '/ycloud/tokenpk/task/step/receivePrize',
            data={
                'marketName': TOKENPK_MARKET_NAME,
                'taskId': task.get('id'),
                'source': 'app',
        ...
        if not data or data.get('code') != 0:
            msg = data.get('msg', '接口无响应') if data else '接口无响应'
            self.log(f'-领取任务奖励失败: {task_name} {msg}')
            return False
        self.log_tokenpk_prizes((data.get('result') or {}).get('prizes'), f'已领取: {task_name}')
        return True

    def complete_tokenpk_upload_photo(self):
        upload_info = self.create_cloud_file('auto_tokenpk_photo_', extension='jpg')
        if not upload_info:
            self.log('-上传照片失败: 接口无响应')
            return False
        self.log(f"-已上传照片: {upload_info.get('fileName', '')}")
        self.cleanup_uploaded_files(upload_info)
        return True

    def complete_tokenpk_action(self, task):
        ext = self.build_tokenpk_task_payload(task).get('key')
        if ext == 'uploadPhoto':
            return self.complete_tokenpk_upload_photo()
        if ext == 'aiCamera':
            return self.complete_ai_camera_task()
        if ext == 'createNote':
            return self.complete_note_task()
        if ext == 'shareFile':
            return self.share_cloud_file()
        if ext == 'inviteFriend':
            invite_data = self.request_tokenpk_json('/ycloud/tokenpk/invite/generateInviteCode') or {}
            if invite_data.get('code') == 0 and invite_data.get('result'):
                return True
            self.log(f"-生成活动邀请码失败: {invite_data.get('msg', '接口无响应')}")
            return False
        if ext in ('openUrl', 'xhsLike'):
            return True
        if ext == 'backup':
            self.log('-需手动完成: 成功备份一次文件')
            return False
        if ext == 'loginPc':
            self.log('-需手动完成: 登录PC客户端上传文件')
            return False
        self.log(f'-暂不支持自动完成: {self.get_tokenpk_task_name(task)}')
        return False

    def reserve_tokenpk_task(self, task):
        task_name = self.get_tokenpk_task_name(task)
        data = self.request_tokenpk_json(
            '/ycloud/tokenpk/task/step/reserve',
            data=self.build_tokenpk_task_payload(task),
            method='POST',
        )
        if not data or data.get('code') != 0:
            msg = data.get('msg', '接口无响应') if data else '接口无响应'
            self.log(f'-预约失败: {task_name} {msg}')
            return False
        prizes = (data.get('result') or {}).get('prizes') or []
        if prizes:
            self.log_tokenpk_prizes(prizes, f'已领取预约奖励: {task_name}')
        else:
            self.log(f'-预约成功: {task_name}')
        return True

    def handle_tokenpk_task(self, task):
        task_name = self.get_tokenpk_task_name(task)
        ...
        if state == 'FINISH':
            ...
            return
        if state == 'SUCCESS':
            if task.get('taskType') == 'RESERVE':
                self.log(f'-已预约: {task_name}')
            else:
                self.receive_tokenpk_task_prize(task)
            return
        if state != 'WAIT':
            self.log(f'-任务状态未知: {task_name} ({state})')
            return
        if task.get('taskType') == 'RESERVE':
            self.reserve_tokenpk_task(task)
            return

        self.log(f'-去完成: {task_name}')
        click_data = self.request_tokenpk_json(
            '/ycloud/tokenpk/task/step/click',
            data=self.build_tokenpk_task_payload(task),
            method='POST',
        )
        if not click_data or click_data.get('code') != 0:
            msg = click_data.get('msg', '接口无响应') if click_data else '接口无响应'
            self.log(f'-任务登记失败: {task_name} {msg}')
            return

        action_completed = self.complete_tokenpk_action(task)
        time.sleep(1)
        refreshed_task = self.get_tokenpk_task(task.get('id')) or task
        refreshed_state = refreshed_task.get('state')
        if refreshed_state == 'SUCCESS':
            self.receive_tokenpk_task_prize(refreshed_task)
        elif refreshed_state == 'FINISH':
            self.log(f'-已完成: {task_name}')
        elif action_completed:
            self.log(f'-已执行任务动作，等待状态更新: {task_name}')

    def receive_pending_tokenpk_task_prizes(self):
        tasks = self.get_tokenpk_task_list(log_error=False) or []
        for task in tasks:
            if task.get('state') == 'SUCCESS' and task.get('taskType') != 'RESERVE':
                self.receive_tokenpk_task_prize(task)
            elif (task.get('state') == 'WAIT'
                  and self.build_tokenpk_task_payload(task).get('key') == 'shareFile'):
                self.log('-分享接口已执行但活动未记账，需在APP内分享一次文件')

    def log_tokenpk_prize_status(self):
        data = self.request_tokenpk_json(
            '/ycloud/prizeApi/checkPrize/getUserPrizeLogPageV2',
            params={
                'marketName': TOKENPK_MARKET_NAME,
                'currPage': 1,
                'pageSize': 1000,
        ...
        if not data or data.get('code') != 0:
            msg = data.get('msg', '接口无响应') if data else '接口无响应'
            self.log(f'-查询算力大作战奖品失败: {msg}')
            return

        records = (data.get('result') or {}).get('result') or []
        pending = [record for record in records if record.get('flag') == 1]
        if pending:
            self.log('\n🎁 算力大作战奖品')
            expiry_times = []
            for record in pending:
                expiry = str(record.get('expireTime') or '').replace('T', ' ').split('.')[0]
                if expiry:
                    expiry_times.append(expiry)
                    self.log(f"-待领取: {record.get('prizeName', '奖品')} ({expiry} 到期)")
                else:
                    self.log(f"-待领取: {record.get('prizeName', '奖品')}")
            if expiry_times:
                self.log(f'-APP领奖 (最早 {min(expiry_times)} 到期): {TOKENPK_PRIZE_PAGE_URL}')
            else:
                self.log(f'-APP领奖: {TOKENPK_PRIZE_PAGE_URL}')
            return

        claimed = [record for record in records if record.get('flag') == 2]
        if claimed:
            self.log('\n🎁 算力大作战奖品')
            for record in claimed[:5]:
                self.log(f"-已领取: {record.get('prizeName', '奖品')}")

    def assist_tokenpk_target(self):
        if not TOKENPK_INVITE_CODE:
            return False
        current_month = datetime.now(timezone(timedelta(hours=8))).strftime('%Y%m')
        stored = get_token_info(self.account)
        if (stored.get('tokenpkAssistCode') == TOKENPK_INVITE_CODE
                and stored.get('tokenpkAssistMonth') == current_month):
            self.log('-本月已完成目标好友助力')
            return False

        own_code_data = self.request_tokenpk_json('/ycloud/tokenpk/invite/generateInviteCode') or {}
        if own_code_data.get('code') != 0 or not own_code_data.get('result'):
            self.log(f"-跳过好友助力: 无法确认本号邀请码 ({own_code_data.get('msg', '接口无响应')})")
            return False
        own_code = str(own_code_data.get('result'))
        if own_code == TOKENPK_INVITE_CODE:
            self.log('-目标邀请码属于本号，跳过自助力')
            return False

        assist_data = self.request_tokenpk_json(
            '/ycloud/tokenpk/invite/acceptInvite',
            data=f'code={TOKENPK_INVITE_CODE}',
            method='POST',
            headers={'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'},
        )
        if assist_data and assist_data.get('code') == 0:
            save_tokenpk_assist_state(self.account, TOKENPK_INVITE_CODE, current_month)
            self.log('-好友助力成功')
            return True
        msg = assist_data.get('msg', '接口无响应') if assist_data else '接口无响应'
        self.log(f'-好友助力未成功: {msg}')
        return False

    def receive_tokenpk_progress_rewards(self):
        home_data = self.request_tokenpk_json('/ycloud/tokenpk/toplist/progress/queryHome')
        if not home_data or home_data.get('code') != 0:
            msg = home_data.get('msg', '接口无响应') if home_data else '接口无响应'
            self.log(f'-查询Token消耗进度失败: {msg}')
            return
        result = home_data.get('result') or {}
        self.log(f"-本月已消耗Token: {result.get('usedToken', 0)}")
        for stage in result.get('rewardStages') or []:
            if stage.get('status') != 1:
                continue
            reward_data = self.request_tokenpk_json(
                '/ycloud/tokenpk/toplist/progress/receiveReward',
                data={'phaseNo': stage.get('phaseNo')},
                method='POST',
            )
            if reward_data and reward_data.get('code') == 0:
                self.log(f"-已领取Token阶段奖励: {stage.get('rewardName', stage.get('phaseNo'))}")
            else:
                msg = reward_data.get('msg', '接口无响应') if reward_data else '接口无响应'
                self.log(f"-Token阶段奖励领取失败: {stage.get('phaseNo')} {msg}")

    def draw_tokenpk_lottery(self):
        chance_data = self.request_tokenpk_json('/ycloud/tokenpk/toplist/progress/queryRemainChance')
        if not chance_data or chance_data.get('code') != 0:
            msg = chance_data.get('msg', '接口无响应') if chance_data else '接口无响应'
            self.log(f'-查询算力抽奖机会失败: {msg}')
            return
        try:
            chances = max(0, int(chance_data.get('result') or 0))
        except (TypeError, ValueError):
            chances = 0
        self.log(f'-当前算力抽奖机会: {chances}')
        for _ in range(chances):
            draw_data = self.request_tokenpk_json(
                '/ycloud/tokenpk/toplist/progress/lottery',
                method='POST',
            )
            if not draw_data or draw_data.get('code') != 0:
                msg = draw_data.get('msg', '接口无响应') if draw_data else '接口无响应'
                self.log(f'-算力抽奖失败: {msg}')
                break
            result = draw_data.get('result') or {}
            if result.get('win'):
                self.log(f"-算力抽奖获得: {result.get('prizeName', '奖品')}")
            else:
                self.log('-算力抽奖: 未中奖')

    @catch_errors
    def national_token_pk(self):
        self.log('\n⚡ 算力大作战')
        tasks = self.get_tokenpk_task_list()
        if tasks is None:
            return
        for task in tasks:
            self.handle_tokenpk_task(task)
        time.sleep(2)
        self.receive_pending_tokenpk_task_prizes()
        self.assist_tokenpk_target()
        self.receive_tokenpk_progress_rewards()
        self.draw_tokenpk_lottery()
        self.log_tokenpk_prize_status()
    ...
    def is_cleanup_upload_file(item):
        ...
        if not (name.endswith('.txt') and (name.startswith('auto_upload_') or name.startswith('auto_share_'))):
            if not (name.endswith('.jpg') and name.startswith('auto_tokenpk_photo_')):
                return False
        size = item.get('size')
    ...
    def get_ai_camera_sample_base64(self):
        ...
        for y in range(height):
            ...
            for x in range(width):
                if (x - 198) ** 2 + (y - 58) ** 2 < 28 ** 2:
                    pixels.extend((245, 166, 35))
                elif y < 166:
                    pixels.extend((82, 151, 210))
                else:
                    pixels.extend((67, 132, 78))
        signature = b'\x89PNG\r\n\x1a\n'
        header = struct.pack('>IIBBBBB', width, height, 8, 2, 0, 0, 0)
        image = (signature + png_chunk(b'IHDR', header)
                 + png_chunk(b'IDAT', zlib.compress(bytes(pixels))) + png_chunk(b'IEND', b''))
    ...
    def complete_ai_camera_task(self):
        ...
        file_name = f'{int(task_id) + 1}.png' if task_id.isdigit() else f'{task_id}.png'
        ...
        if app_type == 'cloud_app':
            if task_type == 'day':
                if task_id == 106:
                    ...
                    self.complete_note_task()



    def complete_note_task(self):
        self.refresh_notetoken()
        if not self.note_token or not self.note_auth:
            self.log('-创建云笔记失败: 获取笔记凭证失败')
            return False
        headers = {
            'X-Tingyun-Id': 'p35OnrDoP8k;c=2;r=1122634489;u=43ee994e8c3a6057970124db00b2442c::8B3D3F05462B6E4C',
            'Charset': 'UTF-8',
        ...
        response = self.send_request(
            'http://mnote.caiyun.feixin.10086.cn/noteServer/api/syncNotebookV3.do',
            headers=headers,
            data={"addNotebooks": [], "delNotebooks": [], "notebookRefs": [], "updateNotebooks": []},
            method='POST',
        )
        if not response:
            self.log('-创建云笔记失败: 获取默认笔记本失败')
            return False
        notebooks = response.json().get('notebooks') or []
        if not notebooks or not notebooks[0].get('notebookId'):
            self.log('-创建云笔记失败: 缺少默认笔记本')
            return False
        self.notebook_id = notebooks[0]['notebookId']
        return self.create_note(headers)
    ...
    def receive(self):
        ...
        rewards = ''
        for value in result:
            market_id = value.get('marketid') or value.get('marketId')
            if value.get('flag') == 1 and market_id != TOKENPK_MARKET_NAME:
